...
public class ProjectionChangeReconciler {
	...
	/**
	 * Using the existing model and a JavaProjectionCalculator instance, determines
	 * the new/old annotations and updates the annotation model accordingly.
	 * 
	 ...
	 * theoretically a fresh scan and construction will yield very similar results
	 * on the positions that do represent the same region. As such, these
	 * annotations are matched carefully and left alone. 2.) Additions: This is any
	 * position found in the new scan that is not matched (as per category 1) with
	 * any position in the current annotation model. 3.) Deletions: This is any
	 * position found in the current annotation model that is not matched (as per
	 * category 1) with any position in the new scan.
	 * 
	 * It is important to recognize the difference between the three categories,
	 * because we must maintain the collapsed/expanded state of each annotation, and
	 * that means matching EXISTING annotations, which contain the user preference
	 * (what the user has collapsed/expanded), with the re-scanned results,
	 * primarily so we don't alter/override the user changes.
	 * 
	 * @param model The projection annotation model containing the current user
	 *              selections.
	 * @param input The element to re-scan.
	 */
	public void reconcile(ProjectionAnnotationModel model, IJavaElement input) {
		...
		try {
			...
			model.modifyAnnotations((ProjectionAnnotation[]) deletions.toArray(new ProjectionAnnotation[0]),
					rebuiltAnnotations, (ProjectionAnnotation[]) updates.toArray(new ProjectionAnnotation[0]));
		} finally {
	 ...
	 * 
	 * @param input The IJavaElement to scan.
	 ...
	 * updated to match the annotation provided by the new scan to ensure that the
	 * annotations match what is expected.
	 * 
	 * @param currentAnnotations The annotations currently in the model (as seen
	 *                           prior the reconciliation process)
	 * @param rebuiltAnnotations The annotations found after rescanning the
	 *                           compilation unit.
	 ...
	 * 
	 * @param model The annotation model (where-in the annotations and positions
	 *              reside).
	 * @return A simplified map data-structure of the data in the annotation model (
	 *         ProjectionAnnotation->Position key->value).
	 */
	 ...
	 * applicable). Some regions may not be foldable due to their size/position (as
	 * determined by user preferences), and as such those positions are removed from
	 * the map.
	 * 
	 * @param positionMap The map containing the positions (as values) to filter and
	 *                    normalize.
	 */
	 ...
	 * 
	 * @param position The position to have its text coordinates normalized
	 * @throws BadLocationException thrown by IDocument queries.
	 */
	 ...
	 * 
	 * Invalid positions commonly occur because the strategies are not
	 * cross-communicative.
	 * 
	 * @author R.J. Lorimer
	 */
	...
	/**
	 * Filters out positions that are not significant enough based on their line
	 * count.
	 * 
	 * @author R.J. Lorimer
	 */
	private class MinimumLineCountFilter extends PositionFilter {
		...
		boolean shouldFilter(Map allPositions, EnhancedPosition position) throws BadLocationException {
			...
			try {
				...
				throw new ReconciliationException("Minimum Line Count Bad Location Exception: Start: " + start
						+ " End: " + end + " User Defined Minimum Height: " + userDefinedMinHeight
						+ " True Minimum Height: " + minimumHeight, e);
			}
		}
	...
	/**
	 * Filters out any user-defined positions that collide with system-defined
	 * positions.
	 * 
	 * This filter is neccessary to prevent the user producing system crossing folds
	 * that confuse the projection viewer API.
	 * 
	 * @author R.J. Lorimer
	 */
	private class UserDefinedCollisionFilter extends PositionFilter {
		boolean shouldFilter(Map allPositions, EnhancedPosition thePosition) throws BadLocationException {
			...
			while (positions.hasNext()) {
				...
				if (aPosition == thePosition)
					continue;
				if (metadata.isUserDefined() && thePosition.collidesWith(aPosition)
						&& !(thePosition.isAdjacent(aPosition))) {
					return true;
				}
			}
	 ...
	 * 
	 * Essentially, due to user-defined regions, two positions may have the same
	 * starting location. This filter attempts to select the best (biggest) position
	 * of the two.
	 * 
	 * @author R.J. Lorimer
	 */
	private class UserDefinedOverrideFilter extends PositionFilter {
		boolean shouldFilter(Map allPositions, EnhancedPosition thePosition) throws BadLocationException {
			...
			while (positions.hasNext()) {
				...
				if (aPosition.getStart() == thePosition.getStart()
						&& aPosition.getLength() >= thePosition.getLength()) {
					return true;
				}
			}
