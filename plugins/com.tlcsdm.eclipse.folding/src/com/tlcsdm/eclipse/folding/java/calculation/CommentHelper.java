...
public class CommentHelper {
	...
	private boolean isUserDefinedSentinel(int token, int start, int end, IJavaElement owner) throws JavaModelException {
		return userDefinedRecognizer.isOpeningSentinel(start, end, owner)
				|| userDefinedRecognizer.isClosingSentinel(start, end, owner);
	}
	...
	private void closeOpenComments() {
		if (lineCommentCount > 1) {
			...
			if (advisor.shouldFoldLineComment()) {
				...
				regions.add(new EnhancedPosition(lineCommentStart, lineCommentEnd - lineCommentStart - 1,
